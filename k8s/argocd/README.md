...
database secrets chart, and applies these Argo CD Applications without waiting
for workload health. `deploy-argocd.yml` can apply and wait for them after image
tag updates or manual dispatch. If you apply this folder manually after a local
Terraform apply, create `openai-secret` first or leave `genai-service` unsynced
